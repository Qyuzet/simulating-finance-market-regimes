...
\IEEEauthorblockA{\textit{School of Computing and Creative Arts} \\
...
The research proposal Contributes to the integration of conformal prediction methods~\cite{barber2022,angelopoulos2021gentle} into the field of financial regime classification. Although the field of conformal prediction has been widely applied to regression and classification tasks~\cite{romano2020classification,angelopoulos2021uncertainty}, the current study is one of the first to specifically address the field of financial regime classification. Based on the current state-of-the-art literature, three open problems are outlined in the field of conformal prediction: (1) the empirical analysis of conformal prediction for financial tasks~\cite{bastos2024conformal,wisniewski2020application}, (2) the computational challenge posed by the issue of unbalanced classes in rarely occurring financial states~\cite{angelopoulos2021class,romano2020classification}, and (3) the processing of non-stationary financial time series data~\cite{xu2022conformal,vovk2020online}. Until now, the current research appears to be the first one to deal with these three problems simultaneously.
...
\textbf{Market Regime Classification.} Regime-switching models have been a major focus area for a very long time in the analysis of financial time series data. Recent advances in machine learning have enabled more sophisticated regime detection methods~\cite{nystrup2015multi,nystrup2017regime}, demonstrating that regime-based allocation strategies can outperform static approaches by adapting to changing market conditions. Hidden Markov models have been particularly effective in the unsupervised identification of regime transitions~\cite{nystrup2015multi}. However, these methods have a disadvantage of not providing risk estimates together with the point forecast.

\textbf{Conformal Prediction.} A theoretical framework for conformal predictions has been proposed based on the concept of exchangeability and distribution-free guarantees~\cite{vovk2005algorithmic,barber2022}. Later studies on conformal predictions include extensions for time series~\cite{xu2021,xu2022conformal,barber2021temporal}, which fail to satisfy the assumption of exchangeability as a consequence of the dependencies among the observed values, and classification settings~\cite{sadinle2019,romano2020classification}. The problem of non-stationarity and changes in the concerned distributions are addressed through adaptive conformal inference~\cite{gibbs2022,gibbs2021adaptive,vovk2020online}. Recent work has also explored online conformal prediction methods~\cite{gibbs2022online} and applications to deep learning~\cite{bae2023deep}. Regarding the use of conformal predictions for the financial regime classification task, a series of issues exist that demand attention, including the evaluation of the empirical coverage for the categorization performance, handling the imbalanced datasets issue, and connection to portfolio-level methods.

\textbf{Uncertainty Quantification in Finance.} Conventional techniques use either the Bayesian approach, which requires prior specification and distributional assumptions, or the bootstrap resampling method, which requires large sample sizes and assumes stationarity. These methods face challenges when addressing the fat-tailed and non-stationary properties of financial returns. Recent advances in distribution-free uncertainty quantification~\cite{barber2021jackknife,angelopoulos2021gentle} have shown promise for financial applications~\cite{bastos2024conformal,wisniewski2020application}. To the best of our knowledge, this study is one of the first to use the conformal prediction method for regime-based portfolio optimization in the literature. We address the current open issues in the literature by proposing a new hybrid solution.

\section{Methodology}

\subsection{Research Flow Overview}

In the following figure, \ref{fig:research_flow}, the full research pipeline for the methodology is depicted with the eight dominant parts of the process for the research presented here. It starts with data gathering and processing and proceeds with the discovery via the use of the HMMs and, finally, the application of the topic to portfolio optimization problems with the methodologies developed. Improved methods, such as Hybrid Conformal Prediction, are then produced to address the instability of the coverage and continue with techniques of generalization over multiple assets and markets.

% FIGURE: Research Flow
\begin{figure}[t]
\centering
\includegraphics[width=\columnwidth]{results/research-flow-compact.png}
\caption{Full research methodology pipeline with the five stages from data acquisition to validated results. The zig-zag layout illustrates the stages from preparing data (blue) to finding regimes (purple) up to conformal prediction and portfolio optimization with validated results in pink and yellow.}
...
\begin{figure}[h]
\centering
\includegraphics[width=\columnwidth]{results/figures/hmm_regime_distribution.png}
...
\begin{figure}[h]
\centering
\includegraphics[width=\columnwidth]{results/figures/regime_characteristics.png}
...
\begin{figure}[h]
\centering
\includegraphics[width=\columnwidth]{results/conformal_prediction/conformal_analysis.png}
...
\begin{figure}[h]
\centering
\includegraphics[width=\columnwidth]{results/conformal_prediction/comparison.png}
...
\begin{figure}[h]
\centering
\includegraphics[width=\columnwidth]{results/cp_portfolio/portfolio_performance.png}
...
\begin{figure}[h]
\centering
\includegraphics[width=\columnwidth]{results/cp_portfolio/allocation_dynamics.png}
...
Improvement & +1.27\% & +3.10\% & -1.47\% & +4.21\% & -66.0\% \\ \bottomrule
\end{tabular}
}
\end{table}

\textbf{Adaptive CP.} Financial time series data are not stationary, exhibiting regime change patterns. Adaptive conformal prediction (CP) methods~\cite{gibbs2021adaptive,gibbs2022online,xu2022conformal} use the concept of a window to update the quantile values when new data points are obtained. Results are obtained using three window values: 126 days, 252 days, and 504 days, equivalent to 6 months, 1 year, and 2 years, respectively. Results in Table \ref{tab:adaptive} show that all window values have achieved valid empirical coverage values between 89.82\% and 90.38\%, and the optimal window value that considers recent regime change patterns and satisfies statistical stability is 126 days.
...
In summary, the performance of each method is presented in the above tables, where it may be observed that the hybrid method achieves the best coverage balance with a standard deviation of 0.95\%, along with strong performance on rare event coverage with bear regime at 91.47\% and neutral regime at 90.76\%.
...
\begin{figure}[h]
\centering
\includegraphics[width=\columnwidth]{results/cp_portfolio/transaction_cost_sensitivity.png}
\caption{Portfolio performance sensitivity analysis across varying transaction cost levels.}
\label{fig:transaction_cost}
\end{figure}

% FIGURE 14: Per-Regime Coverage Heatmap
\begin{figure}[h]
\centering
\includegraphics[width=\columnwidth]{results/multi_asset/per_regime_coverage_heatmap.png}
\caption{Heatmap visualization of coverage rates across regimes and asset classes.}
\label{fig:coverage_heatmap}
\end{figure}

% FIGURE 15: Coverage Balance
\begin{figure}[h]
\centering
\includegraphics[width=\columnwidth]{results/multi_asset/coverage_balance.png}
...
\textbf{Code Availability.} The complete source code and experimental results are publicly available at \href{https://github.com/Qyuzet/simulating-finance-market-regimes}{https://github.com/Qyuzet/simulating-finance-market-regimes}.
...
\textbf{Riki Awal Syahputra}: Conceptualization, methodology, software, formal analysis, writing - original draft. \textbf{Darrus Loamayer}: Data curation, validation, writing - review \& editing. \textbf{Yiyang Liu}: Visualization, investigation, writing - review \& editing. \textbf{Nunung Nurul Qomariyah}: Supervision, project administration. \textbf{Raymond Bahana}: Supervision, resources.

\section*{Acknowledgment}
This work was supported by Bina Nusantara University as part of the Fundamentals of Data Science course (COMP6784001) in the School of Computing and Creative Arts.

\section*{Questions and Answers}

During the presentation, the following questions were raised and addressed:

\textbf{Q1: Why did you choose gold to compare in the multi-asset analysis?}

Gold is chosen as a typical asset class because it displays a distinct régime behavior compared to equity and bond assets. Gold tends to be a safe asset and has a negative correlation with equity assets in a bear régime. Hence, gold is a perfect case study for testing how well our conformal prediction method is capable of adapting to a particular asset class. Gold also has a strong enough historical support base.

\textbf{Q2: Why do you think this project represents a novelty?}

This work tackles the three open issues at the same time: (1) empirical evidence for conformal prediction in financial regime classification, (2) class-conditional approaches to handle class imbalance in rare financial regimes, and (3) conformal prediction for non-stationary financial time series. Our original proposal is a conformal prediction paradigm merging adaptive calibration and class-conditional quantiles, providing a radical improvement of 79.3\% in coverage balance. To the best of our knowledge, no study has combined HMM-regime identification and conformal prediction for portfolio optimization with distribution-free confidence intervals.

\textbf{Q3: Why did you choose FRED as the resource for macroeconomic data?}

The reason for choosing the Federal Reserve Economic Data (FRED) database is that FRED is the most accurate and reliable provider of macroeconomic data in the U.S., and it is provided and maintained by the Federal Reserve Bank of St. Louis. In identifying a country’s regime, macroeconomic variables such as the Federal Funds Rate and Consumer Price Index are essential variables in measuring the cycles of the economy and the trends of the country’s monetary policy.

\textbf{Q4: Why did you choose conformal prediction over other uncertainty quantification methods?}

Rationale for this includes its capability of providing a distribution-free and finite sample guarantee without requiring any assumption on the distribution. This is especially important in financial series, where fat tails, structural changes, and non-stationarity prevail. By using this method, it is possible to provide a guaranteed result for the individual to achieve at least a $1 - \alpha$ probability regardless of its actual probability distribution in a real financial scenario.

\textbf{Q5: Can this framework be implemented for the Indonesian market in the future?}

Yes, the approach is asset agnostic, so it is applicable to the Indonesian market. The methodology needs: (1) enough history to detect the regimes (preferably 10+ years of history), (2) features that are relevant to the market, such as local market variables, along with variables from Bank Indonesia, (3) features that reflect the market specifics, for example, the fact that the market is not 24/7 due to the time difference, which affects the liquidity. The difficulties for the approach to work in the Indonesian market would lie in the ability to find high-quality data for the assets within the market, the possibility that the dynamic patterns for the regimes might differ due to the characteristics of the market (emerging markets, for instance), or the need to take into account the market specifics regarding risk factors.
